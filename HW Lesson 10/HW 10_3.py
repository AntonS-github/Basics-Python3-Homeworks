...
class Cell:
    ...
    def __sub__(self, other):
        dif = other.mesh - self.mesh
        if dif > 0:
            return dif
        else:
            raise ValueError('Less than zero')
    def __mul__(self, other):
        mul = self.mesh * other.mesh
        return mul
    def __truediv__(self, other):
        true_div = self.mesh / other.mesh
        return true_div
    def __floordiv__(self, other):
        floor_div = self.mesh // other.mesh
        return floor_div    
    def make_order(self, num_mesh):
        mesh_list = []
        star = '*'
        for _ in range(self.mesh):
            if _ % num_mesh == 0:
                mesh_list.append('\n')
            mesh_list.append(star)
